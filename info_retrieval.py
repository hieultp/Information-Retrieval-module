...
def augmented_tf(data, dictionary, alpha=0.5, mtype=float):
    ...
    for i in range(aug_tf_var.shape[0]):
        ...
        try:
            aug_tf_var.data[
                aug_tf_var.indptr[i]:aug_tf_var.indptr[i + 1]] /= np.max(
                    aug_tf_var.data[
...
def idf(data, dictionary):
    ...
    for i in range(number_of_features):
        ...
        # datapoint i = data[indptr[i]:indptr[i + 1]]
        matrix_csc.data[
            matrix_csc.indptr[i]:matrix_csc.indptr[i+1]] = np.log(
                N / (1+np.count_nonzero(
                    matrix_csc.data[
    ...
    for i in range(matrix.shape[0]):
        sparse_matrix.data[
            sparse_matrix.indptr[i]:sparse_matrix.indptr[i+1]
                ...
                sparse_matrix.data[
                    sparse_matrix.indptr[i]:sparse_matrix.indptr[i+1]], 2)
